...
2. data: <br />
   ...
   - 2.1 "DisasterResponse.db" is the database which is generated by the process_data.py script
   - 2.2 "categories.csv" holds the different categories of specific ids
   - 2.3 "messages.csv" holds the different messages, originals and genres of specific ids
   - 2.4 process_data.py incorporates the ETL pipeline, which is employed for data cleaning, feature extraction, and storing data in a SQLite database.
3. models: <br />
   contains "classifier.pkl" and train_classifier.py
   - 3.1 contains "classifier.pkl" is the trained model which is generated by the train_classifier.py script
   - 3.2 train_classifier.py includes a machine learning pipeline that loads data, trains a model, and then saves the trained model as a .pkl file for future use.
